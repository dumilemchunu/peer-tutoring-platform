    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user_doc.exists:
                ...
                if user_data.get('role') == 'student' and not user_data.get('is_verified'):
                    flash('Your student account is pending approval by an administrator. You will be notified once approved.', 'warning')
                    return render_template('auth/sign-in.html')
...
def sign_up():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if role == 'student':
                ...
                user_data['is_verified'] = False  # Students need admin approval
                user_data['student_status'] = 'pending'  # Add status field for admin approval
            elif role == 'tutor':
                # Handle tutor fields
                staff_number = request.form.get('staff_number', '')
                user_data['staff_number'] = staff_number
                user_data['qualifications'] = request.form.get('qualifications', '')
                user_data['subjects'] = request.form.get('subjects', '')
                user_data['availability'] = request.form.get('availability', '')
                user_data['tutor_status'] = 'pending'
            ...
            if role == 'student':
                flash('Your student account has been created and is pending admin approval.', 'info')
            else:
    ...
    # If GET request or error, show the sign-up form
    return render_template('auth/sign-up.html')

@auth_bp.route('/sign_up', methods=['GET', 'POST'])
def sign_up_underscore():
    """Alternate route that redirects to the main sign-up route"""
...
def process_signup():
    """
    Direct processing route for the sign-up form
    """
    if request.method != 'POST':
        return redirect(url_for('auth.sign_up'))
    ...
    try:
        ...
        if role == 'student':
            flash('Your student account has been created and is pending admin approval. You will be notified once your account is approved.', 'info')
        else:
